...
const countryRegions: Record<string, { regions: string[]; center: [number, number]; zoom: number }> = {
  Sudan: { regions: ["All Regions", "Khartoum", "North Darfur", "South Darfur", "West Darfur", "Central Darfur", "Blue Nile", "Red Sea", "Kassala"], center: [30.0, 15.5], zoom: 5 },
  Ethiopia: { regions: ["All Regions", "Somali", "Oromia", "Afar", "Amhara", "Tigray", "SNNPR"], center: [40.5, 8.5], zoom: 5.5 },
  "South Sudan": { regions: ["All Regions", "Central Equatoria", "Jonglei", "Unity", "Upper Nile", "Lakes"], center: [31.0, 7.0], zoom: 5.5 },
  Somalia: { regions: ["All Regions", "Banadir", "Bay", "Gedo", "Lower Juba", "Middle Shabelle"], center: [46.0, 5.0], zoom: 5 },
  Yemen: { regions: ["All Regions", "Sana'a", "Aden", "Taiz", "Hodeidah", "Marib"], center: [48.0, 15.5], zoom: 5.5 },
};
...
export default function DetectionPage() {
  ...
  const statCards = [
    ...
    { label: "Data Sources Online", value: `${sources.filter((s) => s.is_active).length}/${sources.length}`, color: "#059669" },
    { label: "Events Today", value: pipelineStats ? formatNumber(pipelineStats.data.recent_24h) : "—", color: undefined },
    { label: "Recent 7 Days", value: String(overview?.recent_7_days ?? "—"), color: undefined },
  ];
    ...
    <Box>
      ...
      <Box px={24} py={12} className="border-b border-[#E5E5E5]" style={{ background: "#FFFFFF" }}>
        ...
        <Group gap={4} mb={8}>
          ...
          <Text size="xs" c="#525252" fw={500}>Detection</Text>
        </Group>

        {/* Title row */}
        <Group gap={12} mb={12}>
          <Text fw={700} c="#171717" style={{ fontSize: 20 }}>Crisis Detection</Text>
        ...
        <Group justify="space-between">
          ...
          <Group gap={8}>
            <Button variant="outline" color="gray" size="xs" leftSection={<IconSearch size={14} />} style={{ fontSize: 13 }}>
              Search
            </Button>
            <Button size="xs" leftSection={<IconPlus size={14} />} style={{ background: "#E85D3D", borderColor: "#E85D3D", fontSize: 13 }}>
              Create Manual Alert
            </Button>
          </Group>
        </Group>
      ...
      <Box p={24}>
        ...
        {activeTab === "live" && (
          <Box style={{ display: "flex", gap: 24 }}>
            ...
            <Box style={{ flex: 1, minWidth: 0 }}>
              {/* EWAS Pipeline Card */}
              <Card p={0} mb={24} style={{ border: "1px solid #2563EB", background: "linear-gradient(135deg, #F0F9FF 0%, #E0F2FE 100%)" }}>
                <Box px={16} py={12} className="border-b border-[#2563EB30]">
                  <Group justify="space-between">
                    <Group gap={8}>
                      <IconDatabase size={16} color="#2563EB" />
                      <Text fw={600} c="#171717" style={{ fontSize: 14 }}>EWAS Sudan Pipeline</Text>
                      <Badge size="xs" style={{ background: "#D1FAE5", color: "#059669" }}>CONNECTED</Badge>
                    </Group>
                    <Group gap={8}>
                      <Text size="xs" c="#737373">Last sync: {pipelineStats ? "2 min ago" : "\u2014"}</Text>
                      <Button
                        size="xs"
                        variant="subtle"
                        color="blue"
                ...
                <Box p={16}>
                  <Group gap={12} mb={12}>
                    <Box>
                      <Text size="xs" c="#737373">Sources</Text>
                      <Text fw={700} c="#2563EB" style={{ fontSize: 20 }}>{pipelineStats?.sources.total ?? sources.length}</Text>
                    </Box>
                    <Box>
                      <Text size="xs" c="#737373">Total Records</Text>
                      <Text fw={700} c="#171717" style={{ fontSize: 20 }}>{pipelineStats ? formatNumber(pipelineStats.data.total_records) : "\u2014"}</Text>
                    </Box>
                    <Box>
                      <Text size="xs" c="#737373">Status</Text>
                      <Group gap={4}>
                        <IconPointFilled size={10} color="#059669" />
                        <Text fw={600} size="sm" c="#059669">
                          {sources.every((s) => s.is_active) ? "All Online" : `${sources.filter((s) => s.is_active).length} Online`}
                        </Text>
                      </Group>
                    </Box>
                  </Group>
                  <Group gap={8} wrap="wrap">
                    {sources.map((src) => (
                      <Text key={src.id} size="xs" px={8} py={4} fw={500} style={{ background: src.is_active ? "#2563EB15" : "#D9770615", color: src.is_active ? "#2563EB" : "#D97706", border: `1px solid ${src.is_active ? "#2563EB30" : "#D9770630"}` }}>
              ...
              {/* Active Alerts */}
              <Card p={0} style={{ border: "1px solid #E5E5E5", display: "flex", flexDirection: "column" }}>
                <Box px={16} py={12} className="border-b border-[#E5E5E5]" style={{ flexShrink: 0 }}>
                  <Group justify="space-between">
                    <Box>
                      <Text fw={600} c="#171717" style={{ fontSize: 14 }}>Active Alerts ({alerts.length})</Text>
                      <Text size="xs" c="#737373">{selectedCountry}{selectedRegion !== "All Regions" ? ` — ${selectedRegion}` : ""} — sorted by severity</Text>
                    </Box>
                ...
                {alerts.length === 0 && !alertsQuery.isLoading && (
                  <Box px={16} py={32} style={{ textAlign: "center" }}>
                    <Text c="#737373" size="sm">No active alerts at this time.</Text>
                  </Box>
                )}
                {alerts.map((alert) => {
                  ...
                  return (
                    <Link key={alert.id} href={`/crisis/${alert.id}`} style={{ textDecoration: "none", color: "inherit" }}>
                      <Box px={16} py={12} className="border-b border-[#E5E5E5] hover:bg-[#F9FAFB] cursor-pointer" style={{ display: "flex", gap: 12 }}>
                        <Box style={{ width: 4, background: sevColor, flexShrink: 0 }} />
                        <Box style={{ flex: 1 }}>
                          <Group justify="space-between" mb={4}>
                            <Group gap={8}>
                              <Badge size="xs" variant="light" style={{ background: sevBg, color: sevColor, fontWeight: 600 }}>
                                {severityLabels[sev]}
                              </Badge>
                              {alert.data_source && (
                                <Text size="xs" px={6} py={2} style={{ background: `${sevColor}15`, color: sevColor, fontSize: 10 }}>
                                  {alert.data_source.name}
                                </Text>
                              )}
                            </Group>
                            <Text size="xs" c="#A3A3A3">{formatTimeAgo(alert.updated_at)}</Text>
                          </Group>
                          <Text fw={600} size="sm" c="#171717" mb={2}>{alert.title}</Text>
                          <Text size="xs" c="#737373" mb={8} lineClamp={1}>{location ? `${location.name}` : ""} {alert.shock_type ? `\u2022 ${alert.shock_type.name}` : ""}</Text>
                          <Group gap={16}>
                            <Text size="xs" c="#737373">
                              <Text span size="xs" c="#737373">Severity: </Text>
                              <Text span size="xs" fw={500} c="#171717">{alert.severity}/5</Text>
                            </Text>
                            <Text size="xs" c="#737373">
                              <Text span size="xs" c="#737373">Shock date: </Text>
                              <Text span size="xs" fw={500} c="#171717">{new Date(alert.shock_date).toLocaleDateString()}</Text>
                            </Text>
                          </Group>
                        </Box>
                      </Box>
        ...
        {activeTab === "sources" && (
          <Card p={0} style={{ border: "1px solid #E5E5E5" }}>
            <Box px={16} py={12} className="border-b border-[#E5E5E5]">
              <Group justify="space-between">
                <Box>
                  <Text fw={600} c="#171717" style={{ fontSize: 14 }}>Connected Data Sources</Text>
                  <Text size="xs" c="#737373">{sources.length} sources feeding the detection engine</Text>
                </Box>
                <Button size="xs" variant="outline" color="gray">+ Connect Source</Button>
              </Group>
            </Box>
            {sourcesQuery.isLoading ? (
              <Box p={32} style={{ textAlign: "center" }}><Loader size="sm" /></Box>
            ) : (
              <Table>
                <Table.Thead>
                  <Table.Tr style={{ background: "#F5F5F5" }}>
                    <Table.Th style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px", color: "#737373", fontWeight: 600 }}>Source</Table.Th>
                    ...
                    <Table.Th style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px", color: "#737373", fontWeight: 600 }}>Frequency</Table.Th>
                    <Table.Th style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px", color: "#737373", fontWeight: 600 }}>Variables</Table.Th>
                  </Table.Tr>
                </Table.Thead>
                <Table.Tbody>
                  {sources.map((src) => (
                    <Table.Tr key={src.id}>
                      <Table.Td>
                        <Box>
                          <Text fw={600} style={{ fontSize: 13 }}>{src.name}</Text>
                          {src.description && <Text size="xs" c="#737373" lineClamp={1}>{src.description}</Text>}
                        </Box>
                      </Table.Td>
                      <Table.Td><Text c="#525252" style={{ fontSize: 13 }}>{src.type}</Text></Table.Td>
                      <Table.Td>
                        <Group gap={6}>
                          <IconPointFilled size={10} color={src.is_active ? "#059669" : "#D97706"} />
                          <Text c="#525252" style={{ fontSize: 13 }}>{src.is_active ? "Online" : "Offline"}</Text>
                        </Group>
                      </Table.Td>
                      <Table.Td><Text c="#525252" style={{ fontSize: 13 }}>{src.data_frequency}</Text></Table.Td>
                      <Table.Td><Text c="#525252" style={{ fontSize: 13 }}>{src.variable_count ?? "—"}</Text></Table.Td>
                    </Table.Tr>
                  ))}
                </Table.Tbody>
              </Table>
        ...
        {activeTab === "rules" && (
          <Card p={0} style={{ border: "1px solid #E5E5E5" }}>
            <Box px={16} py={12} className="border-b border-[#E5E5E5]">
              <Group justify="space-between">
                <Box>
                  <Text fw={600} c="#171717" style={{ fontSize: 14 }}>Automated Alert Rules (Detectors)</Text>
                  <Text size="xs" c="#737373">
            ...
            {detectorsQuery.isLoading ? (
              ...
              detectors.map((detector, i) => (
                <Box key={detector.id} px={16} py={14} className={i < detectors.length - 1 ? "border-b border-[#E5E5E5]" : ""}>
                  <Group justify="space-between">
                    <Box style={{ flex: 1 }}>
                      <Group gap={8} mb={4}>
                        <Text fw={600} size="sm" c="#171717">{detector.name}</Text>
                        <Badge size="xs" style={{ background: "#EFF6FF", color: "#2563EB" }}>
                      ...
                      {detector.last_run && (
                        <Text size="xs" c="#A3A3A3" mt={2}>Last run: {formatTimeAgo(detector.last_run)}</Text>
                      )}
                    </Box>
                    <Switch
                      checked={ruleStates[detector.name] ?? detector.active}
                      onChange={() => setRuleStates((prev) => ({ ...prev, [detector.name]: !(prev[detector.name] ?? detector.active) }))}
                      color="#E85D3D"
                      size="sm"
                    />
                  </Group>
        ...
        {activeTab === "history" && (
          <Card p={0} style={{ border: "1px solid #E5E5E5" }}>
            <Box px={16} py={12} className="border-b border-[#E5E5E5]">
              <Box>
                <Text fw={600} c="#171717" style={{ fontSize: 14 }}>Alert History</Text>
                <Text size="xs" c="#737373">
                  {historyQuery.data ? `${historyQuery.data.total ?? historyQuery.data.count} total alerts` : "Past alerts"}
                </Text>
              </Box>
            </Box>
            {historyQuery.isLoading ? (
              ...
              <Table>
                <Table.Thead>
                  <Table.Tr style={{ background: "#F5F5F5" }}>
                    <Table.Th style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px", color: "#737373", fontWeight: 600 }}>Alert</Table.Th>
                    <Table.Th style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px", color: "#737373", fontWeight: 600 }}>Severity</Table.Th>
                    <Table.Th style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px", color: "#737373", fontWeight: 600 }}>Type</Table.Th>
                    <Table.Th style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px", color: "#737373", fontWeight: 600 }}>Date</Table.Th>
                  ...
                  {historyAlerts.map((alert) => {
                    ...
                    return (
                      <Table.Tr key={alert.id}>
                        <Table.Td>
                          <Link href={`/crisis/${alert.id}`} style={{ textDecoration: "none" }}>
                            <Text fw={600} style={{ fontSize: 13, color: "#171717" }}>{alert.title}</Text>
                          </Link>
                        </Table.Td>
                        <Table.Td>
                          <Badge size="xs" style={{ background: sevBg, color: sevColor }}>{severityLabels[sev]}</Badge>
                        </Table.Td>
                        <Table.Td><Text c="#525252" style={{ fontSize: 13 }}>{alert.shock_type?.name ?? "—"}</Text></Table.Td>
                        <Table.Td><Text c="#525252" style={{ fontSize: 13 }}>{new Date(alert.shock_date).toLocaleDateString()}</Text></Table.Td>
                        <Table.Td><Text c="#525252" style={{ fontSize: 13 }}>{alert.locations?.[0]?.name ?? "—"}</Text></Table.Td>
                      </Table.Tr>
                    );
                  })}
                </Table.Tbody>
