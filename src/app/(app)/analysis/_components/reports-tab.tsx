...
import { situationReports } from "./analysis-data";
        ...
        {!crisesQuery.isLoading && crises.length === 0 &&
          situationReports.map((report, idx) => {
            ...
            return (
              ...
              >
                <Box style={{ width: 12, height: 12, background: report.markerColor, flexShrink: 0, marginTop: 4 }} />
                <Box style={{ flex: 1, minWidth: 0 }}>
                  <Group justify="space-between" mb={6} gap={12} align="flex-start">
                    <Text fw={700} size="sm" c="var(--color-text-primary)">{report.title}</Text>
                    <Badge
                      size="xs"
                      style={{
                  ...
                  <Text size="sm" c="var(--color-text-secondary)" mb={12} style={{ lineHeight: 1.6 }}>{report.description}</Text>
                  {pills.length > 0 && (
                    <Group gap={6} mb={12}>
                      {pills.map((pill) => (
                        <Badge key={pill.label} size="xs" style={{ background: pill.bg, color: pill.color, fontWeight: 600, fontSize: 10 }}>
