...
// ─── Paginated queries (alerts / events / signals) + stats ─────────────────
...
// Shared zod schemas for the filter shape. Kept loose (z.string() for ids,
// arbitrary string enums for orderBy) so callers can pass through values
// without us having to mirror every API enum in two places.
  ...
  signalsPage: protectedProcedure
    .input(
      z.object({
        ...
        // Signals filter shape matches commonFilter except `eventTypes` is
